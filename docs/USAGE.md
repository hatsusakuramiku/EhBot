...
「设置 → 系统」的四项运行参数存在数据库里，改完下一次请求即生效，**不需要重启、也不写进 `.env`**：
...
- **页面不改进程级别**。`LOG_LEVEL` 仍然只在环境变量里设：级别若存在于界面，就无法用它去排查界面起来之前就失败的启动。因此把页面调到比 `LOG_LEVEL` 更低的级别不会多出记录，页面会直接标出当前进程级别是哪一个。
- **内存缓冲 + 文件兜底**。最新 1000 条留在内存里，所以 `LOG_TO_FILE=false` 或数据目录只读时页面照样有内容——那恰好是最需要看日志的部署。进程刚启动、内存里还没有该级别的记录时，改读 `<data>/logs/ehbot.log`，页面会说明这一次读的是磁盘。
...
| 变量 | 默认 | 作用 |
|------|------|------|
| `LOG_LEVEL` | `INFO` | 全局级别，调至 `DEBUG` 可看详细警告、交互上下文。`/logs` 页面的级别只筛选显示，不改这一项 |
