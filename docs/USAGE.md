...
- **打包前一定先补元数据**：归档路径、文件名和整个 `ComicInfo.xml` 都是从元数据推出来的，而成品路径随后会写进产物记录。因此只要候选带 eh 链接而元数据还缺，打包会**先刮一次再动手**；否则书会以 `candidate-57.cbz` 落库，事后补刮也搬不回来。刮取失败不会卡住下载：仍按已有标题打包，并记一条日志。
