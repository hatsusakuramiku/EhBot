...
> **2026-09-08 后续调整**：运行等级改为 WebUI 持久设置，`LOG_LEVEL` 仅作数据库打开前的启动默认值；
> `DEBUG` 开启访问日志，其余等级关闭；文件改为 UTC 日切并将 `WARNING` 及以上复制到每日错误日志。
> 下文关于 `LOG_ACCESS`、`RotatingFileHandler` 和 `ehbot.log` 的内容保留为原方案历史，不再描述当前实现。
